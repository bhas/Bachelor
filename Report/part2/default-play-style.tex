...
The second input is the number of opponents. The second input is normalized as \[I_{2} = \frac{Opp}{Opp_{max}}\]
...
\begin{figure}[H]
  ...
  \caption{TNE graph from training the second ANN.\label{fig:finaltest}}
\end{figure}

To further test whether or not the ANN can make profitable choices based on the training with the data from the player mayor, we choose to use a test-bed for poker bots called Poker Genius. Here we are able to test the ANN against the poker bots of Poker Genius. We manually recreated the choices the ANN made against the bots of Poker Genius. It was done manually, because we did not manage to get Poker Genius implementation of personal bots to work. As shown in figure \ref{fig:nn2} it is clear that the ANN is not able to compete on the same level as the bots from Poker Genius.
...
The first problem is related to the data we use. The problem is that  the data is not complete since we do not have the data about the hole cards of the irrelevant players. This means that we can not learn the ANN when to fold, as we have no input for the hole cards. On the other hand do we really need this information to create a default strategy? Poker is a game of imperfect information and human players are able to play a game of poker without learning from an experienced player so should the computer not have that ability too? 
There are two solutions, one is to collect a complete dataset with all the information and disregard that poker should be a game of imperfect information. Another is to program the procedure for determining when to fold. 
...
The second problem was to determine the number of inputs for the ANN. We chose five inputs that we found relevant. Perhaps we overlooked some inputs that could be interesting to look at. It is also possible that one or more of the inputs simply are confusing the ANN more than it is helping figuring out a pattern in the players strategy. 


The final problem we encountered was defining our requirements for the ANN. We do not know what TNE is realistic and our requirements may be unrealistic. Whether or not the TNE is somewhat fair is hard to determine. But it seems like we would have to get a much lower TNE in order to create a realistic strategy from the data of the player.\\

Learning a default strategy based on random players in random rounds are perhaps not the best option. However we were not able to retrieve any game data from a series of professional poker players.
...
Another issue is that in order for a player to be successful the player has to be good at adapting to the opponents strategy. This would mean that the ANN would have a hard time learning the strategy of the player as the player probably changed strategy often.

The ANN were not able to make profitable choices when playing against the bots of Poker Genius. But it is rather hard to determine whether or not this is because of its lack to learn from the player or due to the player not making wise decisions. We wonder if the result would be different if we had had data from a professional poker player.


Ville slette det her og ned og bruge det ovenover... Hva synes du?


The second problem was to determine the number of inputs for the ANN. Five relevant inputs was chosen for our ANN. Since most players adapt their strategy to strategies of the opponents, it is close to impossible to learn the strategy of the player without knowing the strategy of the opponents. 


It was a bad idea to try learn a default strategy from random players in random rounds. The players had most likely already gained information about the opponents and thus adapted their strategy to the opponents. It would have made more sense to observe players who just joined a table to see how they played in a situation where they had not adapted to the opponents yet.
...
We try to develop the strategy by creating different ANN approaches and train them using data from real-life poker games. The last two approaches were able to learn from a player but unfortunately the ANN's ability to learn quickly dropped.\\

For the final ANN we chose three randomly picked players. All the actions and information about the game in the moment of the action were collected to use in the training of the ANN. The TNE of each player specific ANN was compared to the hope of having a TNE of five percent or less. The best ANN had a TNE of $\sim$14.0~\% which is not what we hoped for. However the ANN is able to learn from the players to some degree. 
It goes without saying that a complete dataset with the hole cards of all the players in the game would benefit the ANN's ability to learn how to handle each situation.
